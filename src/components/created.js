...
query wallet($address: String) {
  ...
  created: holdings(where: {_and: [{holder_address: {_eq: $address}}, {token: {_and: [{minter_address: {_eq: $address}}, {_or: [{tags: {tag: {_ilike: "%pixel%"}}}, {fa2_address: {_eq: "KT1MxDwChiDwd6WBVs24g1NjERUoK622ZEFp"}}]}]}}]}, order_by: {token: {minted_at: desc}}) {
    token {
      artifact_uri
      fa2_address
      mime_type
...
//   }, [])
  ...
  const { data, error } = useSWR(address && ['/api/created', getObjkts, address], fetcher, { refreshInterval: 15000 })
...
console.log(data, banned.length)
console.log('why?',banned.length)
  const created = data?.created.filter((i) => !banned.includes(i.token.artist_address))
