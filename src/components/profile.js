...
query walletName($address: String) {
    alias: holdings(where: {token: {tags: {tag: {_ilike: "%pixel%"}}}, holder_profile: {alias: {_ilike: $address}}}, order_by: {token: {minted_at: desc}}) {
      token {
        artifact_uri
        artist_address
        fa2_address
    ...
    aliasBidou: holdings(where: {holder_profile: {alias: {_ilike: $address}}, fa2_address: {_eq: "KT1MxDwChiDwd6WBVs24g1NjERUoK622ZEFp"}}, order_by: {token: {minted_at: desc}}) {
        token {
          artifact_uri
          artist_address
          fa2_address
    ...
    pk:  holdings(where: {holder_profile: {account: {_eq: $address}}, token: {tags: {tag: {_ilike: "%pixel%"}}}}, order_by: {token: {minted_at: desc}}) {
        token {
          artifact_uri
          artist_address
          fa2_address
    ...
    pkBidou:  holdings(where: {holder_profile: {account: {_eq: $address}}, fa2_address: {_eq: "KT1MxDwChiDwd6WBVs24g1NjERUoK622ZEFp"}}, order_by: {token: {minted_at: desc}}) {
        token {
          artifact_uri
          artist_address
          fa2_address
...
//   }, [])
  ...
  const final = owned?.filter((i) => !banned.includes(i.token.artist_address))
