...
export const LatestSales = () => {
  ...
  const banned = async() => await axios.get('https://raw.githubusercontent.com/hicetnunc2000/hicetnunc-reports/main/filters/w.json');
  const final = data?.tokens.filter((i) => !banned?.data?.includes(i.artist_address))
